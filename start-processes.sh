...
tmux new-window -t db-compare-dev:2 -n server -c $root
tmux send-keys -t db-compare-dev:2 'cd db-compare-server/ && ./dev-build.sh' Enter

tmux new-window -t db-compare-dev:3 -n react-client -c $root
tmux send-keys -t db-compare-dev:3 'cd react-client && npm run start' Enter 
